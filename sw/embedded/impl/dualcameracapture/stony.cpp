...
//void camera::init(short vref,short nbias,short aobias,char gain,char selamp)
void camera::init( char inPinRESP, char inPinINCP, char inPinRESV, char inPinINCV,
                   char inPinINPHI, char inPinANALOG1, char inPinANALOG2,
                   short vref, short nbias, short aobias, char gain, char selamp )
{
  short config;

/* TODO russ: use later
  if(camera::MAX_GAIN < inGain)
  {
    return camera::RC_ERROR_BADPARAM;
  }
...
*/
  ...
  setPointerValue(camera::REG_CONFIG,config);
...
void camera::setPointer(char ptr)
{
  short ii;
...
void camera::setValue(short val)
{
  short ii;
...
void camera::incValue(short val)
{
  short ii;
...
// brings it low.
void camera::pulseInphi(char delay)
{
...
void camera::setPointerValue(char ptr, short val)
{
...
void camera::clearValues(void)
{
  short ii;

  for (ii = 0; ii < camera::REG_CNT; ++ii)
  {
...
void  camera::setVREF(short vref)
{
  setPointerValue(camera::REG_VREF,vref);
}

// setNBIAS
// Sets the NBIAS register value (0-63)
void  camera::setNBIAS(short nbias)
{
  setPointerValue(camera::REG_NBIAS,nbias);
}

// setAOBIAS
// Sets the AOBIAS register value (0-63)
void  camera::setAOBIAS(short aobias)
{
  setPointerValue(camera::REG_AOBIAS,aobias);
}

// setBiasesVdd
// Sets biases based on chip voltage
void camera::setBiasesVdd(char flagVddType5v5)
{
  ...
  switch (flagVddType5v5)
  {
    ...
    default:
      setPointerValue(camera::REG_NBIAS,camera::VAL_NBIAS_5V0);
      setPointerValue(camera::REG_AOBIAS,camera::VAL_AOBIAS_5V0);
      setPointerValue(camera::REG_VREF,camera::VAL_VREF_5V0);
    break;
  }
}
...
void camera::setBiases(short vref,short nbias,short aobias)
{
  setPointerValue(camera::REG_NBIAS,nbias);
  setPointerValue(camera::REG_AOBIAS,aobias);
  setPointerValue(camera::REG_VREF,vref);
...
// setConfig(4,1,1);
void camera::setConfig(char gain, char selamp, char cvdda)
{
   short config;
...
void camera::setAmpGain(char gain)
{
  short config;

  if((gain>0) && (gain<=camera::MAX_GAIN))  //if gain is a proper value, connect amp
  {
  ...
  setPointerValue(camera::REG_CONFIG,config);  //set config register
}
...
void camera::getImage( short *img, unsigned char rowstart, unsigned char numrows,
                       ...
                       unsigned char colskip ) //, char ADCType, char anain )
{
  ...
  // Go to first row
  setPointerValue(camera::REG_ROWSEL,rowstart);

  // Loop through all rows
  for (row=0; row<numrows; ++row)
  {
    // Go to first column
    setPointerValue(camera::REG_COLSEL,colstart);
    ...
    setPointer(camera::REG_ROWSEL);
    incValue(rowskip); // go to next row
  }
}

// russ: dual read!
void camera::getDualImages( short *img1, short* img2, unsigned char rowstart, unsigned char numrows,
                            unsigned char rowskip, unsigned char colstart, unsigned char numcols,
...
*/
  ...
  Serial.println("Img = [");
  setPointerValue(camera::REG_ROWSEL,0); // set row = 0
  for (row=0; row<rows; ++row)
  {
    setPointerValue(camera::REG_COLSEL,0); // set column = 0
...
*/
    ...
    setPointer(camera::REG_ROWSEL); // point to row
